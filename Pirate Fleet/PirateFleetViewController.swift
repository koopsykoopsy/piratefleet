...
extension PirateFleetViewController: PlayerDelegate {
    ...
    func playerDidWin(player: Player) {
        ...
        if gameOver == false {
            ...
            // human won!
            case .Human:
                createAlertWithTitle(Settings.Messages.WinTitle, message: Settings.Messages.GameOverWin + String(human.calculateScore(computer)) + ".", actionMessage: Settings.Messages.ResetAction, completionHandler: { (action) in self.initializeGame() })
            
            // computer won!
            case .Computer:
                createAlertWithTitle(Settings.Messages.LoseTitle, message: Settings.Messages.GameOverLose + String(human.calculateScore(computer)) + ".", actionMessage: Settings.Messages.ResetAction, completionHandler: { (action) in self.initializeGame() })
            }
            
            // print final score for debugging
